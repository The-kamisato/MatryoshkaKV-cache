...
class PcaLlamaDistillationTrainer(PcaLlamaTrainer):
    ...
    # def compute_loss(self, model, inputs, return_outputs=False, T=1.0):
    #     with torch.no_grad():
    #         teacher_logits = self.source_model(**inputs)["logits"]
        
    #     if model.training:
    #         inputs["key_truncate_index"] = 32 * torch.randint(1, 5, (32, 32), dtype=torch.long)
    #         inputs["value_truncate_index"] = 32 * torch.randint(1, 5, (32, 32), dtype=torch.long)
    ...
    #     # teacher_logits = torch.randn_like(outputs["logits"]).to(outputs["logits"].device)
    ...
    #     # print("soft_targets.shape", soft_targets.shape)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, T=1.0, soft_target_loss_weight=0.25, ce_loss_weight=0.75):
        ...
        if model.training:
            ...
            #     # inputs["value_truncate_index"] = 16 * torch.randint(1, 9, (32, 32), dtype=torch.long)
            # else:
            #     possible_values = random.choice([32, 64, 96, 128])
            #     inputs["key_truncate_index"] = possible_values * torch.full((32, 32), 1, dtype=torch.long)
            #     inputs["value_truncate_index"] = possible_values * torch.full((32, 32), 1, dtype=torch.long)
        else:
            inputs["key_truncate_index"] = torch.full((32, 32), 64, dtype=torch.long)
            inputs["value_truncate_index"] = torch.full((32, 32), 64, dtype=torch.long)
...
class PcaGemmaDistillationTrainer(PcaLlamaTrainer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        print("Loading source model:")
        self.source_model_config = AutoConfig.from_pretrained("/liymai24/sjtu/bokai/PCA_kvcache/checkpoint/gemma-2b", cache_dir=False)
        self.source_model = GemmaForCausalLM.from_pretrained("/liymai24/sjtu/bokai/PCA_kvcache/checkpoint/gemma-2b", torch_dtype=torch.bfloat16).to(self.model.device)
        print("Gemma Load over!!!")
    
    def compute_loss(self, model, inputs, return_outputs=False, T=50.0, soft_target_loss_weight=0.25, ce_loss_weight=0.75):
        with torch.no_grad():
            teacher_outputs = self.source_model(**inputs)
            teacher_logits = teacher_outputs["logits"]
            teacher_loss = teacher_outputs["loss"]

        if model.training:
            # inputs["key_truncate_index"] = 64 * torch.randint(1, 5, (18, 8), dtype=torch.long)
            # inputs["value_truncate_index"] = 64 * torch.randint(1, 5, (18, 8), dtype=torch.long)

            inputs["key_truncate_index"] = 32 * torch.randint(1, 9, (18, 8), dtype=torch.long)
            inputs["value_truncate_index"] = 32 * torch.randint(1, 9, (18, 8), dtype=torch.long)

            # inputs["key_truncate_index"] = torch.full((18, 8), 256, dtype=torch.long)
            # inputs["value_truncate_index"] = torch.full((18, 8), 256, dtype=torch.long)
        else:
            inputs["key_truncate_index"] = torch.full((18, 8), 128, dtype=torch.long)
            inputs["value_truncate_index"] = torch.full((18, 8), 128, dtype=torch.long)
    ...
    def compute_eval_loss(self, model, inputs, return_outputs=False, T=50.0):
        with torch.no_grad():
            teacher_logits = self.source_model(**inputs)["logits"]
