...
@dataclass
class BaseModelOutputWithPastAndPenalty(BaseModelOutputWithPast):
    """
    ...
    Args:
        ...
        attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            heads.
    """
    all_layers_penalty: List[torch.FloatTensor] = None
...
class PcaLlamaAttention(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        key_covariance = torch.matmul(key_unitary_transform_matrix[..., :key_truncate_index].transpose(-1, -2), key_unitary_transform_matrix[..., :key_truncate_index])     # (32, 128, 128)
        value_covariance = torch.matmul(value_unitary_transform_matrix[..., :value_truncate_index].transpose(-1, -2), value_unitary_transform_matrix[..., :value_truncate_index])     # (32, 128, 128)
        I_key_matrix = torch.eye(key_truncate_index).unsqueeze(0).repeat(self.config.num_attention_heads, 1, 1).to(hidden_states.device)
        I_value_matrix = torch.eye(value_truncate_index).unsqueeze(0).repeat(self.config.num_attention_heads, 1, 1).to(hidden_states.device)
        key_frob_distances = torch.norm(key_covariance - I_key_matrix, dim=(1, 2))
        value_frob_distances = torch.norm(value_covariance - I_value_matrix, dim=(1, 2))
        penalty = (torch.mean(key_frob_distances) + torch.mean(value_frob_distances)) / 2

        return attn_output, attn_weights, past_key_value, penalty
...
class PcaLlamaDecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        # Self Attention
        hidden_states, self_attn_weights, present_key_value, penalty = self.self_attn(
...
class PcaLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
    ) -> Union[Tuple, BaseModelOutputWithPastAndPenalty]:
        ...
        for decoder_layer in self.layers:
            ...
            all_layers_penalty.append(layer_outputs[-1])
        ...
        return BaseModelOutputWithPastAndPenalty(
            last_hidden_state=hidden_states,
            past_key_values=next_cache,
            hidden_states=all_hidden_states,
            attentions=all_self_attns,
            all_layers_penalty=all_layers_penalty,
        )
...
class PcaLlamaForCausalLM(LlamaForCausalLM):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            penalty = torch.mean(torch.stack(all_layers_penalty), dim=0)
