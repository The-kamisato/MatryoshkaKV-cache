...
def change_label_piqa(input_ids, label):
    first_A_or_B_index = ((label == 350) | (label == 319)).nonzero(as_tuple=True)[0].item()
    first_2_index = (label == 2).nonzero(as_tuple=True)[0].item()
    end_index = min(first_A_or_B_index, first_2_index)
...
class PcaLlamaAttention(LlamaAttention):
    def __init__(
        ...
        self.key_scaling = nn.Parameter(torch.ones(self.key_unitary_transform_weights.shape[0], self.key_unitary_transform_weights.shape[-1])) if train_key else None  
        self.value_scaling = nn.Parameter(torch.ones(self.value_unitary_transform_weights.shape[0], self.value_unitary_transform_weights.shape[-1])) if train_value else None
    ...
    def forward(
        ...
        **kwargs,
        ...
        key_unitary_transform_matrix = self.orthogonal_(self.key_unitary_transform_weights, orthogonal_type='cayley') * self.key_scaling if self.train_key else self.key_unitary_transform_matrix
        value_unitary_transform_matrix = self.orthogonal_(self.value_unitary_transform_weights, orthogonal_type='cayley') * self.value_scaling if self.train_value else self.value_unitary_transform_matrix
